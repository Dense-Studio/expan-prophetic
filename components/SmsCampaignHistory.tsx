                  ...
                  <button key={campaign.id} type="button" onClick={() => openCampaign(campaign)} className="w-full rounded-xl border border-brand/10 bg-white p-4 text-left hover:border-brand/30 hover:shadow-sm transition-all">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2"><span className="text-sm font-bold text-brand-dark">{campaign.kind === "live" ? "Live Broadcast" : campaign.kind === "general" ? "Generic Broadcast" : "Programme Reminder"}</span><span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide ${statusClass(campaign.status)}`}>{STATUS_LABELS[campaign.status] || campaign.status}</span>{campaign.sandbox && <span className="px-2 py-1 rounded-full bg-violet-100 text-violet-700 text-[10px] font-bold">SANDBOX</span>}</div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-brand/50"><span>{campaign.estimated_credits.toLocaleString()} credits</span><span>{campaign.sms_parts} part{campaign.sms_parts === 1 ? "" : "s"}</span>{campaign.priority_recipients > 0 && <span>{campaign.priority_recipients.toLocaleString()} auditorium priority</span>}<span>{countValue(campaign.counts, "failed") + countValue(campaign.counts, "not_delivered") + countValue(campaign.counts, "expired")} failed</span><span>{countValue(campaign.counts, "needs_review")} need review</span></div>
                  </button>
                );
              })}
            </div>
            ...
            <div className="p-5 sm:p-7 overflow-y-auto space-y-5">
              ...
              <div className="rounded-xl border border-brand/10 bg-white overflow-x-auto">
                {detailLoading ? <div className="py-12 flex justify-center"><span className="spinner spinner-dark" /></div> : (
                  <table className="w-full text-left text-xs min-w-[820px]"><thead className="bg-brand/5 text-brand/60 uppercase tracking-wider text-[9px]"><tr><th className="px-4 py-3">Phone</th><th className="px-4 py-3">Priority</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Attempts</th><th className="px-4 py-3">Arkesel ID</th><th className="px-4 py-3">Error</th><th className="px-4 py-3">Updated</th></tr></thead><tbody className="divide-y divide-brand/5">{recipients.map((recipient) => <tr key={recipient.id}><td className="px-4 py-3 font-medium text-brand-dark">{recipient.normalized_phone || recipient.original_phone}</td><td className="px-4 py-3">{recipient.priority_tier === 1 ? <span className="px-2 py-1 rounded-full bg-violet-100 text-violet-700 font-bold">Auditorium</span> : "Standard"}</td><td className="px-4 py-3"><span className={`px-2 py-1 rounded-full font-bold ${statusClass(recipient.status)}`}>{recipient.status.replace("_", " ")}</span></td><td className="px-4 py-3">{recipient.attempt_count}</td><td className="px-4 py-3 font-mono text-[10px] text-brand/50">{recipient.provider_message_id || "-"}</td><td className="px-4 py-3 text-red-600 max-w-xs truncate" title={recipient.error_message || ""}>{recipient.error_code || recipient.error_message || "-"}</td><td className="px-4 py-3 text-brand/45">{new Date(recipient.updated_at).toLocaleString()}</td></tr>)}</tbody></table>
                )}
